...
def train_loop(model, optimizer, n_checkpoints, X_train, y_train, X_test, y_test, D, start_epoch, n_epochs, batch_size, model_results_path):
    ...
    for epoch_i in range(start_epoch, n_epochs):
        ...
        if epoch_i % (1 if n_epochs < 100 else (10 if n_epochs <= 1000 else 100)) == 0:
            ...
            print(f'Epoch {epoch_i}, Train loss: {loss.item():.3e}, Test loss: {loss_test.item():.3e}, MSE(e): {e_loss.item():.3e}, MSE(pi1): {pi1_loss.item():.3e}, MSE(pi2): {pi2_loss.item():.3e}, MSE(pi3): {pi3_loss.item():.3e}')
