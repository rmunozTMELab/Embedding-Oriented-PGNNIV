...
class DataGenerator:
    ...
    def _expand_dims(self, g1, g2, X, Y):
        """
    ...
    def u_func(self, g1, g2, X, Y):
        ...
        """
        g1, g2, X, _ = self._expand_dims(g1, g2, X, Y)  # Expand dimensions for calculations
        return g1 + (g2 - g1) * X  # Calculate u based on g1 and g2

    def qx_func(self, g1, g2, X, Y):
        """
        Computes the qx function based on inputs g1 and g2.
        ...
        """
        g1, g2, X, _ = self._expand_dims(g1, g2, X, Y)  # Expand dimensions for calculations
        return (g2 - g1) * np.ones_like(X)  # Calculate qx based on g1 and g2

    def qy_func(self, g1, g2, X, Y):
        """
        Computes the qy function based on inputs g1 and g2.
        ...
        """
        g1, g2, X, _ = self._expand_dims(g1, g2, X, Y)  # Expand dimensions for calculations
        return (g2 - g1) * np.zeros_like(X)  # Calculate qy based on g1 and g2 (will result in zero array)

    def k_func(self, g1, g2, X, Y):
        """
        Computes a function 'k' based on inputs g1 and g2.
        ...
        """
        g1, g2, X, _ = self._expand_dims(g1, g2, X, Y)  # Expand dimensions for calculations
        return np.ones_like(g1 + (g2 - g1) * X)  # Calculate k (returns an array of ones)
    ...
    def generate_dataset(self, test_size=0.3):
        ...
        u = self.u_func(self.g1, self.g2, self.X_mesh, self.Y_mesh)
        qx = self.qx_func(self.g1, self.g2, self.X_mesh, self.Y_mesh)
        qy = self.qy_func(self.g1, self.g2, self.X_mesh, self.Y_mesh)
        k = self.k_func(self.g1, self.g2, self.X_mesh, self.Y_mesh)
        ...
        split_data = train_test_split(X, y, self.g1, self.g2, qx, qy, u, k, test_size=test_size, random_state=self.custom_seed)
        (X_train, X_val, y_train, y_val, g1_train, g1_val, g2_train, g2_val, qx_train, qx_val, qy_train, qy_val, u_train, u_val, k_train, k_val) = split_data
